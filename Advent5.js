...
while (!halt) {
    ...
    var inst = parseInst(read);
    console.log('inst', inst);
    console.log('i', i);
    var val1 = (inst[1] === 0 ? arr[arr[i+1]]: arr[i+1]);
    var val2 = (inst[2] === 0 ? arr[arr[i+2]]: arr[i+2]);
    var val3 = (inst[3] === 0 ? arr[arr[i+3]]: arr[i+3]);
    console.log('vals', val1, val2, val3);
    switch(inst[0]) {
        case 1:
        ...
        case 3:
            var inp = 5;
            arr[arr[i+1]] = inp;
            console.log('input', inp, 'now in location', arr[i+1]);
            i += 2;
            break;
        case 4:
            ...
            break;
        case 5:
            if (val1 !== 0) { i = val2; } else { i += 3; }
            break;
        case 6:
            if (val1 === 0) { i = val2; } else { i += 3; }
            break;
        case 7:
            arr[arr[i+3]] = (val1 < val2 ? 1 : 0);
            i += 4;
            break;
        case 8:
            arr[arr[i+3]] = (val1 === val2 ? 1 : 0);
...
function parseInst(n) {
    var digits = (""+n).split("").map((s) => Number(s));
    var opcode = (digits[digits.length-2]*10+digits[digits.length-1] || digits[digits.length-1]);
    var cmode = (digits[digits.length-3] || 0);
    var bmode = (digits[digits.length-4] || 0);
    var amode = (digits[digits.length-5] || 0);
    return [opcode, cmode, bmode, amode];
}