...
def run(str, m):
    ...
    for inst in instructions:
        ...
        thedist = int(inst[1:])
        if thedir == 'L':
            m[coord[0]-thedist:coord[0], coord[1]] = 1
            coord[0] -= thedist
        if thedir == 'R':
            m[coord[0]:coord[0]+thedist, coord[1]] = 1
            coord[0] += thedist
        if thedir == 'D':
            m[coord[0], coord[1]-thedist:coord[1]] = 1
            coord[1] -= thedist
        if thedir == 'U':
            m[coord[0], coord[1]:coord[1]+thedist] = 1
            coord[1] += thedist
        print(coord)
...
def run2(str, m):
    ...
    for inst in instructions:
        ...
        thedist = int(inst[1:])
        if thedir == 'L':
            inta = (m[coord[0]-thedist:coord[0], coord[1]])
            intinds = (np.where(inta==2))
            # print(intinds)
            for intind in intinds[0]:
                print(coord[0]-thedist+intind, coord[1], nsteps+(thedist - intind))
            coord[0] -= thedist
            nsteps += thedist

        if thedir == 'R':
            inta = (m[coord[0]:coord[0]+thedist, coord[1]])
            intinds = (np.where(inta==2))
            # print(intinds)
            for intind in intinds[0]:
                print(coord[0]+intind, coord[1], nsteps+intind)
            coord[0] += thedist
            nsteps += thedist

        if thedir == 'D':
            inta = (m[coord[0], coord[1]-thedist:coord[1]])
            intinds = (np.where(inta==2))
            # print(intinds)
            for intind in intinds[0]:
                print(coord[0], coord[1]-thedist+intind, nsteps+(thedist - intind))
            coord[1] -= thedist
            nsteps += thedist

        if thedir == 'U':
            inta = (m[coord[0], coord[1]:coord[1]+thedist])
            intinds = (np.where(inta==2))
            # print(intinds)
            for intind in intinds[0]:
                print(coord[0], coord[1]+intind, nsteps+intind)
            coord[1] += thedist
            nsteps += thedist
        # print(coord)
